...
class PostListTest(APITestCase):
    ...
    def testPostListGET(self):
        ...
        # Populate list
        response = self.client.post(str(self.author.data["url"]) +"/posts/", self.post_data,  format="json")
    
        # Check status is ok
        response = self.client.get(str(self.author.data["url"]) +"/posts/")
...
class PostDetailsTest(APITestCase):
    ...
    def testPostDetailsPUT(self):
        ...
        response = self.client.put("/authors/" + str(self.author1.uuid) +"/posts/"+new_post_id+'/', self.put_data, format="json")
        # check status is 201
        self.assertEqual(response.status_code, 201)
    ...
    def testPostDetailsPOST(self):
        ...
        response = self.client.post("/authors/" + str(self.author1.uuid) +"/posts/"+str(new_post.uuid)+'/', self.post_data_edit, format="json")
